...
const getNthElementOfProperty = rewire('./app.js').__get__(
...
test('Function getNthElementOfProperty must return a number', () => {
  expect(typeof getNthElementOfProperty([1, 2], 'key', 1)).toBe('number') ||
    expect(typeof getNthElementOfProperty([1, 'test'], 'key', 1)).toBe(
      'string'
    ) ||
    expect(typeof getNthElementOfProperty([1, { a: 1 }], 'key', 1)).toBe(
      'object'
    );
});
