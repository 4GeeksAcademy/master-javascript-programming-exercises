...
const getAverageOfElementsAtProperty = rewire('./app.js').__get__(
...
test('Function getAverageOfElementsAtProperty must return something', () => {
  expect(getAverageOfElementsAtProperty({ key: [4, 8, 6] }, 'key')).not.toBe(
    undefined
  );
});
test('Function getAverageOfElementsAtProperty must return a number', () => {
  expect(
    typeof getAverageOfElementsAtProperty({ key: [4, 8, 6] }, 'key')
  ).not.toBe('number');
});
