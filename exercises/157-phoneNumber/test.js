...
test('Function getAreaCode must exist on PhoneNumberFormatter', () => {
  expect(new PhoneNumberFormatter([]).getAreaCode).not.toBe(undefined);
});
test('Function getExchangeCode must exist on PhoneNumberFormatter', () => {
  expect(new PhoneNumberFormatter([]).getExchangeCode).not.toBe(undefined);
});
test('Function getLineNumber must exist on PhoneNumberFormatter', () => {
  expect(new PhoneNumberFormatter([]).getLineNumber).not.toBe(undefined);
});
test('Function parenthesize must exist on PhoneNumberFormatter', () => {
  expect(new PhoneNumberFormatter([]).parenthesize).not.toBe(undefined);
});
test('Function slice must exist on PhoneNumberFormatter', () => {
  expect(new PhoneNumberFormatter([]).slice).not.toBe(undefined);
});
